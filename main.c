...
void generatePlayers(int n, int initEnergy)  		
{
	...
	for (i=0; i<smm_player_nr; i++)
	{
		...
		smm_players[i].flag_graduated = 0;	 			// energy initial value = set initial energy
 ...
 //print all players status at the beginning of each turn
void printPlayerStatus(void)  
{
	int i;
	
	//print all players position, credit, energy
	for(i=0; i<smm_player_nr; i++)
	{
		printf("%s - position : %i (%s), credit : %i, energy : %i\n",
					smm_players[i].name, smm_players[i].pos, smmObj_getNodeName(smm_players[i].pos), 
